...
def header_exists(page, name):
    ...
    if formula_lib._header_pos(page, name):
        return True
    ...
    return bool(formula_lib._header_pos(page, name))
    ...
    while time.time() < deadline:
        try:
            ...
            if not auto or auto in KNOWN_HEADERS or auto.startswith("Rows from"):
                ...
                continue
            formula_lib.rename_column(page, auto, name)
            return
        except Exception as e:
            last_err = e
            page.wait_for_timeout(2000)
...
def column_status(page, name):
    ...
    pos = formula_lib._header_pos(page, name)
    if not pos:
        return None
    js = """(x) => {
      for (const el of document.querySelectorAll('*')) {
...
def delete_column(page, name):
    ...
    pos = formula_lib.header_click_pos(page, name)
    if not pos:
...
def recover_leftover(page, prefixes, name, attempts=4, allow=()):
    ...
    for attempt in range(attempts):
        ...
        if target is None:
            return False
        try:
            formula_lib.rename_column(page, target, name)
            return True
        except Exception as e:
            last = e
            page.keyboard.press("Escape")
