...
recipe). Usage:  python build_event.py --folder "Forum Labo"
...
REPO_ROOT = os.path.normpath(os.path.join(common.SCRIPT_DIR, "..", ".."))
...
ROLLOUT_SHOTS = os.path.join(common.SCRIPT_DIR, "rollout_shots")
...
3. Special-case rules (Step C):
   ...
   - Investors & VC: classify as Buyer -> "Investors & Venture Capital" for funds/PE/VC when titles like Partner/Principal/Analyst/Associate are indicated; this overrides Step B.
...
class EventBuilder:
    ...
    def rename_new(self, name, table, prefixes, allow=()):
        ...
        try:
            B.rename_last_column(self.page, name, timeout_s=45)
            return
        except Exception as e:
            self.say(f"WARN rename {name} flaked ({str(e)[:80]}); re-navigating")
        B.open_workbook(self.page, self.folder)
        B.focus_table_maybe_empty(self.page, table)
        if B.recover_leftover(self.page, prefixes, name, allow=allow):
        ...
        unknown = [h for h in B.unknown_headers(self.page)
                   if not h.startswith("Send")]
        if not unknown:
            raise B.GateError(f"{name}: not found after re-navigation")
        if len(unknown) > 1:
            self.say(f"WARN multiple unknown columns {unknown}; renaming"
                     f" newest -> {name}; strays need manual cleanup")
        import formula_lib as _fl
        _fl.rename_column(self.page, unknown[-1], name)
    ...
    def step_import(self):
        ...
        if not os.path.exists(csv_path):
            raise B.GateError("no Exhibitors_normalized.csv")
        last = None
        for attempt in range(4):
            try:
                ...
                if B.table_exists(page, "Exhibitors_normalized"):
                    self.say("normalized table already present")
                    return
                B.add_csv_table_robust(page, csv_path)
        ...
        raise B.GateError(f"import failed after retries: {last}")
    ...
    def formula(self, name, desc, gate, allow_empty_preview=True):
        ...
        for h in B.unknown_headers(page):
            if h.startswith("Send"):
                continue
            if B.column_status(page, h) == "100%":
                cells = formula_lib.preview_cells(page, n=5, header=h) or []
                located = [c for c in cells if c is not None]
                if located and not any(c for c in located if c and c != "None"):
                    B.delete_column(page, h)
        ...
        for attempt in range(3):
            if f.strip():
                break
            try:
                f, preview = formula_lib.build_formula_column(
        ...
        if not f.strip():
            raise B.GateError(f"{name}: formula generation returned empty 3x")
        ...
        if not ok and not (allow_empty_preview and not nonempty and why == "preview"):
            formula_lib.cancel_panel(page)
            raise B.GateError(f"{name}: gate failed ({why}); formula={f[:200]!r}")
        formula_lib.save_column(page, f"f_{slug(name)}_save")
        try:
            B.rename_last_column(page, name)
        except Exception as e:
            ...
            try:
                formula_lib.scroll_grid_right(page)
                auto = formula_lib.last_header(page)
                if auto and auto not in B.KNOWN_HEADERS                         and not auto.startswith("Rows from"):
                    B.delete_column(page, auto)
            except Exception:
                pass
            raise B.GateError(f"{name}: rename failed; column removed for retry")
        ...
        for _ in range(36):                    # up to 3 min for slow computes
            status = B.column_status(page, name)
            if status == "":
                break
        ...
        if status == "100%":
            ...
            for _ in range(3):                 # re-sample; computing cells fill in
                cells = formula_lib.preview_cells(page, n=6, header=name) or []
            ...
            if verdict == "healthy":
                ...
                B.delete_column(page, name)
                raise B.GateError(f"{name}: formula errored on rows "
    ...
    def step_official_domain(self):
        page = self.page
        if B.header_exists(page, "Official Domain"):
            self.say("column exists: Official Domain")
            return
        if B.recover_leftover(page, ("Primary Corporate", "Corporate Domain",
                                     "Verified Corporate"), "Official Domain"):
            self.say("recovered leftover claygent -> Official Domain")
            return
        B.open_card(page, "Claygent", ("Claygent (AI Web Researcher)",),
                    must_not=("Create a column",))
        B.set_model_gpt41mini(page)
        txt = B.fill_prompt(page, OFFICIAL_DOMAIN_PROMPT,
                            [("Company Name: ", "Company Name"),
                             ("Country: ", "Country")])
        if "UNKNOWN" not in txt or "Company Name" not in txt:
            raise B.GateError("official domain prompt incomplete")
        B.auto_run_off(page)
        B.add_run_condition(page, "!", "Website")
        B.save_plain(page)
    ...
    def step_enrich(self):
        page = self.page
        if B.header_exists(page, "Enrich Company"):
            self.say("column exists: Enrich Company")
            return
        B.open_card(page, "Enrich Company", ("Enrich Company", "Companies, People"))
        B.auto_run_off(page)
        ...
        if not search.count():
            raise B.GateError("enrich field search not found")
        for term in ("Size", "Type", "Domain", "Url", "Founded", "Industry",
                     "Description", "Annual revenue"):
            search.fill(term)
            page.wait_for_timeout(1100)
            for r in page.evaluate(B.MAP_SCAN.replace("[role=\"checkbox\"]",
                                                      "[role=\"switch\"]")):
                lbl = r["label"] or ""
        ...
        B.save_plain(page)
        if not B.header_exists(page, "Enrich Company"):
            raise B.GateError("Enrich Company column missing after save")
        self.snap("06_enrich")
    ...
    def step_registrar(self):
        page = self.page
        if B.header_exists(page, "Labs Series Registrar"):
            self.say("column exists: Labs Series Registrar")
            return
        if B.recover_leftover(page, ("Side:", "Classification", "Side"),
        ...
        B.open_card(page, "Claygent", ("Claygent (AI Web Researcher)",),
                    must_not=("Create a column",))
        B.set_model_gpt41mini(page)
        txt = B.fill_prompt(page, REGISTRAR_PROMPT,
                            [("Company Name: ", "Company Name"),
        ...
        for marker in ("#CONTEXT#", "#EXAMPLES#", "Forensics & Security",
            ...
            if marker not in txt:
                raise B.GateError(f"registrar prompt missing {marker!r}")
        B.auto_run_off(page)   # collapses Configuration, revealing outputs
        if not B.rename_response_output(page, "Side"):
            raise B.GateError("response output not found")
        B.add_output(page, "Classification")
        B.save_plain(page)
    ...
    def step_ref_tables(self):
        ...
        for name in ("fit_lookup", "seller_sublevels", "seller_contact_titles",
            ...
            if B.table_exists(page, name):
                continue
            path = os.path.join(REF_DIR, name + ".csv")
            B.add_csv_table_robust(page, path)
            self.say(f"ref table imported: {name}")
        B.focus_table(page, "Exhibitors_normalized")

    # ---------------------------------------------------------------- sends
    def step_send_blocklist(self):
        page = self.page
        if B.header_exists(page, "Send to Blocklist"):
            self.say("send exists: Blocklist")
            return
        if B.recover_leftover(page, ("Send table data",), "Send to Blocklist"):
            self.say("recovered unrenamed send -> Send to Blocklist")
            return
        target = blocklist_route.ensure_target(page, log=self.log)
        self.say(f"blocklist target: {target[-1]}")
        B.open_workbook(page, self.folder)
        B.focus_table(page, "Exhibitors_normalized")
        B.open_send_panel(page)
        B.dest_existing(page, target)
        extras = B.set_mapping(page, {"Company Name", "Company Domain"})
        if extras:
            self.say(f"WARN blocklist mapping kept locked extras: {extras}")
        B.save_via_menu(page, r"Save and run .* rows in this view")
    ...
    def _split(self, side, table):
        page = self.page
        col = f"Send to {table}"
        if B.header_exists(page, col):
            self.say(f"send exists: {col}")
            return
        if B.recover_leftover(page, ("Send table data",), col):
            self.say(f"recovered unrenamed send -> {col}")
            return
        for attempt in range(2):
            B.open_send_panel(page)
            if B.table_exists(page, table):
                B.dest_existing(page, [table])
            else:
                B.dest_create_table(page, table)
            B.set_mapping(page, SEND_KEEP, required=SEND_KEEP - {"Composite Tier"})
            B.add_run_condition(page, "", "Side", f' == "{side}"')
            B.save_via_menu(page, r"Save and don't run")
            try:
                self.rename_new(col, "Exhibitors_normalized", ("Send table data",))
                break
            except B.GateError as e:
    ...
    def step_splits(self):
        B.focus_table(self.page, "Exhibitors_normalized")
    ...
    def step_sublevel(self):
        page = self.page
        B.focus_table_maybe_empty(page, "Sellers")
        if B.header_exists(page, "Sub Level"):
        ...
        if "Forensics & Security" not in txt or "TAXONOMY" not in txt:
            raise B.GateError("sub level prompt incomplete")
        B.rename_panel_title(page, "Sub Level")
        B.auto_run_off(page)   # collapses Configuration, revealing outputs
        B.rename_response_output(page, "Sub Level")
        B.add_run_condition(page, "!!", "Classification")
        B.save_plain(page)
    ...
    def step_routes(self):
        ...
        B.focus_table_maybe_empty(page, "Sellers")
        if not B.header_exists(page, "Send to Contacts") and \
                B.recover_leftover(page, ("Send table data",), "Send to Contacts"):
            self.say("recovered unrenamed send -> Send to Contacts (Sellers)")
        if not B.header_exists(page, "Send to Contacts"):
            B.open_send_panel(page)
            if B.table_exists(page, "Contacts – Sellers"):
                B.dest_existing(page, ["Contacts – Sellers"])
            else:
                B.dest_create_table(page, "Contacts – Sellers")
            B.set_mapping(page, ROUTE_KEEP | {"Sub Level"}, required=ROUTE_KEEP)
            B.add_run_condition(page, "[1,2].includes(", "Composite Tier", ")")
            B.save_via_menu(page, r"Save and don't run")
            self.rename_new("Send to Contacts", "Sellers", ("Send table data",))
            self.say("Sellers route created")
        # Buyers -> Contacts – Buyers (all rows)
        B.focus_table_maybe_empty(page, "Buyers")
        if not B.header_exists(page, "Send to Contacts") and \
                B.recover_leftover(page, ("Send table data",), "Send to Contacts"):
            self.say("recovered unrenamed send -> Send to Contacts (Buyers)")
        if not B.header_exists(page, "Send to Contacts"):
            B.open_send_panel(page)
            if B.table_exists(page, "Contacts – Buyers"):
                B.dest_existing(page, ["Contacts – Buyers"])
            else:
                B.dest_create_table(page, "Contacts – Buyers")
            B.set_mapping(page, ROUTE_KEEP - {"Composite Tier"},
    ...
    def step_contacts(self):
        page = self.page
        B.focus_table_maybe_empty(page, "Contacts – Sellers")
        self.formula(
            "Sector Keyword Match",
            ...
            lambda f, pv: (all(k in f.lower() for k in
                           ...
                           "formula"))
        B.focus_table_maybe_empty(page, "Contacts – Buyers")
        for attempt in range(3):
            if B.header_exists(page, "JT Fit"):
                break
            # recover an unnamed leftover from a prior attempt first
            recovered = False
            for cand in ("New Column", "Text", "Untitled"):
                if formula_lib._header_pos(page, cand):
                    try:
                        formula_lib.rename_column(page, cand, "JT Fit")
                        recovered = True
                    except Exception as e:
                        ...
                        page.wait_for_timeout(3000)
                    break
            if recovered or formula_lib._header_pos(page, "JT Fit"):
                continue
            page.keyboard.press("Escape")
            ...
            page.wait_for_timeout(800)
        if not B.header_exists(page, "JT Fit"):
            raise B.GateError("JT Fit column not created")
    ...
    def _fp_search(self, table, name, desc, min_filters):
        ...
        B.open_card(page, "Find people at these companies",
        ...
        for i in range(page.locator("textarea").count()):
            ...
            if bb and bb["x"] > 900:
                ...
                break
        if ta is None:
            raise B.GateError("FP chat textarea not found")
        ...
        for _ in range(60):                 # sculptor can take minutes under load
            ...
            if page.evaluate(js):
                ...
                break
        if not done:
            raise B.GateError("FP sculptor never completed")
        js2 = """() => {
          for (const el of document.querySelectorAll('*')) {
            if (el.children.length === 0) {
        ...
        if nf < min_filters:
            raise B.GateError(f"FP filters too few: {nf} < {min_filters}")
        ...
        for i in range(page.locator("input").count()):
            ...
            if bb and 600 < bb["x"] < 1100 and 280 < bb["y"] < 330:
                ...
                break
        if inp is None:
            raise B.GateError("FP save-search name input not found")
    ...
    def run(self):
        t0 = time.time()
        self.step_import()
        B.open_workbook(self.page, self.folder, table=None)
        B.focus_table(self.page, "Exhibitors_normalized")
