...
- `build_automation/` — copy of the original
  ...
  `build_lib.py`), the blocklist-send helper (`blocklist_route.py`), the
  per-event build driver (`build_event.py`), the multi-event fleet rollout
  driver (`rollout.py`), and the persistent-worker/queue mechanism
  (`worker.py`, `wait_out.py`).
...
`build_automation/build_event.py`):
...
`build_automation/common.py`'s `CLAY_SYNC_DIR` was also updated to point at
...
3. `python build_automation/build_event.py --folder "<Event Folder Name>"`
   to build one event's pipeline (guarded by existence checks — safe to
   rerun if interrupted).
4. `python build_automation/rollout.py` to run `build_event` across every
...
teaching `build_event.py` to read `config/entity-types/<entity>.yaml` and
`config/icps/<icp>/icp.yaml` directly — the same data
`template/render_build_prompt.py` already reads — instead of its current
hardcoded Interphex/Exhibitors assumptions, so `build_event.py --entity
