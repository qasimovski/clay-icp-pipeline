...
add_workemail_waterfall.py (Waterfall > WORK EMAIL). Run this only after
the waterfall has finished, since the gate and the Person's Email mapping both
reference the column it produces.

  python add_validate_email.py <wid> <name> --recon
  python add_validate_email.py <wid> <name> --dry-run
  python add_validate_email.py <wid> <name>
"""
...
def _dump(page, say, note):
    say(f"  --- panel ({note}) ---")
    for t in panel._panel_text(page):
        say(f"    {t!r}")
...
def add_validate(page, entry, dry_run, say, recon=False, shot_dir=None):
    ...
    colcfg.open_card(page, "Validate email", must_contain=CARD_MUST_CONTAIN,
    ...
    saved = commit_fields(page, say) or panel.save_column(page, say)
    page.wait_for_timeout(3000)
    ...
    for col in list(KEEP_FIELDS) + ["Validate email"]:
        if clay_ui._find_header_rect(page, col):
            try:
                verify = panel.verify_persisted(page, col, SOURCE_COLUMN, say)
                break
            except Exception as e:
...
def _open_field_picker(page, say):
    ...
    panel.open_column_config(page, "Validate email")
...
def fix_fields(page, entry, say):
    ...
    verify = None
    try:
        verify = panel.verify_persisted(page, "Validate email", SOURCE_COLUMN, say)
    except Exception as e:
