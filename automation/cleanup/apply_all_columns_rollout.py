...
workbooks (reuses cols_manifest.json). Idempotent (event skips already-applied).
  ...
  python apply_all_columns_rollout.py --only "Interphex"
  python apply_all_columns_rollout.py --limit 10        # next 10 not-yet-applied
  python apply_all_columns_rollout.py --dry-run --limit 10
"""
...
def main():
    ...
    with browser_session.clay_page(headless=not args.headed) as page, \
        ...
        for i, entry in enumerate(pending):
            ...
            try:
                r = apply_all_columns.apply_v1(page, entry, args.dry_run, say)
                results.append(r)
