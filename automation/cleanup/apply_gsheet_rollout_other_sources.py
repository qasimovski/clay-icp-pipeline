...
errored). Unlike apply_gsheet_rollout.py, this folder isn't an entity/ICP
...
apply_gsheet_event.py's _wait_for_full_completion) before considering it
...
tables can still be processing rows well after that first read.
  ...
  python apply_gsheet_rollout_other_sources.py --only "EIAC Directory"
  python apply_gsheet_rollout_other_sources.py --limit 5
  python apply_gsheet_rollout_other_sources.py --dry-run --limit 5
"""
