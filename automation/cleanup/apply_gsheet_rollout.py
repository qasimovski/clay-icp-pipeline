...
Entity/ICP-agnostic (--entity/--icp), same pattern as people_rollout.py /
buyer_rollout.py — this is step (g) in the per-event pipeline, run after
people_rollout.py (e) and buyer_rollout.py (f) since it targets the tables
...
"Lookup in Audiences" signature column (see apply_gsheet_event.py).

  python apply_gsheet_rollout.py --only "Analytica India"      # default exhibitors/labs
  python apply_gsheet_rollout.py --limit 10
  python apply_gsheet_rollout.py --entity sponsors --limit 10
  python apply_gsheet_rollout.py --dry-run --limit 5
"""
...
import apply_gsheet_event as A  # noqa: E402
...
# build_cols_manifest.py since the slug-namespacing convention landed — the
...
def main():
    ...
    cfg = PC.load(args.entity, args.icp)
    slug = cfg.slug()
    ...
    if not target_ids:
        raise SystemExit(
            ...
            f"people_rollout.py/buyer_rollout.py's targets-building step first.")
        ...
        for i, entry in enumerate(pending):
            ...
            for table in tables:
                ...
                for dns_try in range(3):
                    try:
                        r = A.apply_gsheet(page, entry, table, args.dry_run, say)
                        last_exc = None
                        break
