...
chip inside the Body JSON ("Name": {{Name}}), a few scrolls down.
  ...
  python fix_supabase_api.py --audit                 # read-only, all 9
  python fix_supabase_api.py --only "Material Sciences"
  python fix_supabase_api.py --limit 9               # fix + run
"""
...
def _open_cfg(page, say, tries=4):
    ...
    for i in range(tries):
        try:
            panel.open_column_config(page, COLUMN)
