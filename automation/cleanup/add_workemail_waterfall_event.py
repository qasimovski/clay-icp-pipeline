...
{{WORK EMAIL}} in the next step's condition resolves without a rename.
  ...
  python add_workemail_waterfall_event.py <wid> <name> --recon
  python add_workemail_waterfall_event.py <wid> <name> --dry-run
  python add_workemail_waterfall_event.py <wid> <name>
"""
...
# --------------------------------------------------------------------------
# canonical column-config helpers (fix_workemail_gate.py imports these)
# --------------------------------------------------------------------------
...
def open_column_config(page, column):
    ...
    if not find_header_scrolling(page, column):
        raise B.GateError(f"column {column!r} not found")
    ...
    for el in page.get_by_role("menuitem").all():
        try:
            ...
            pass
    raise B.GateError("'Edit column' menu item not found")
...
def auto_run_off(page, say):
    ...
    for row in page.evaluate(_SWITCH_ROWS):
        ...
        if now in ("true", "checked"):
            raise B.GateError("auto-run switch would not turn off")
...
def verify_persisted(page, column, gate_column, say):
    ...
    for attempt in range(4):
        ...
        raise B.GateError(f"could not reopen {column!r} to verify: {last}")
...
def trigger_run(page, say):
    ...
    trigger (see run_v1_event.py); rows failing the run condition are skipped
...
def column_progress(page, column):
    ...
    try:
        return B.column_status(page, column)
    except Exception:
        return None
...
def _set_gate_condition(page, column, say):
    ...
    lab = page.get_by_text("Add run condition", exact=True)
    if not lab.count():
        raise B.GateError("run condition label not visible")
    lb = lab.first.bounding_box()
    cb = None
    for el in page.locator('[role="checkbox"]').all():
        try:
        ...
        if bb and bb["x"] > 1240 and abs((bb["y"] + bb["height"] / 2) -
            ...
            break
    if cb is None:
        raise B.GateError("run condition checkbox not found")
    ...
    pick = _cond_pick(page, label_y)
    if not pick:
        raise B.GateError("condition editor not found")
    ...
    for attempt in range(3):
        if not _focus_condition_editor(page, label_y, say):
            raise B.GateError("could not focus the condition editor")
    ...
    raise B.GateError(f"condition never became {want!r}")
...
def repair_gate(page, column, gate_column, say):
    ...
    Generalised from fix_workemail_gate.py: any save path can drop a run
...
def add_waterfall(page, entry, dry_run, say, recon=False, run_after=False):
    ...
    B.open_card(page, "Waterfall", must_contain=CARD_MUST_CONTAIN,
    ...
    # !!{{Speaker Name}} — build_lib types the "!!" then inserts /<column>.
    try:
