...
"<entity>_<icp>" slug so different entities never share state/targets/logs:
  ...
  python buyer_people_rollout.py --only "Pittcon"                 # default entity=exhibitors icp=labs
  python buyer_people_rollout.py --limit 5
  python buyer_people_rollout.py --entity sponsors --limit 5      # run the same flow for Sponsors
"""
...
def main():
    ...
    cfg = buyer_people_searches.configure(args.entity, args.icp)   # point the build at this entity/ICP
    slug = cfg.slug()
