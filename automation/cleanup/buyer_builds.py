...
Reuses people_builds (open Find people, fill seniority/job-titles/countries/limit,
save-new/save-existing) with PEOPLE_TABLE swapped to 'Buyers - People', and
apply_filters_event for the filter-panel primitives.
"""
...
import people_builds as PB   # noqa: E402
import apply_filters_event as F  # noqa: E402
...
# direct use (e.g. buyer_rollout --only) works without an explicit call.
...
def configure(entity=None, icp=None):
    ...
    PB.TABLE = cfg.main_table   # shared open_find_people focuses this source table
    ...
    PB.PEOPLE_TABLE = cfg.buyer_people_table   # save into the buyer table
    return cfg
...
def _open_filter_panel(page):
    ...
    is visible."""
    for _ in range(6):
        if F._row_y(page, "Where", tries=1):
            return True
    ...
    return bool(F._row_y(page, "Where", tries=2))
...
def set_side_buyer(page, say):
    ...
    y1 = F._row_y(page, "Where")
    if y1 is None:
        raise clay_ui.ClayUIError("Side (Where) row not found")
    val = page.evaluate(F._VAL_AT, y1)
    if not val:
...
def ensure_classification_filter(page, say):
    ...
    if op:
        ...
        if "contains any of" not in cur.lower():
            ...
            if not PB._click_exact(page, "contains any of", timeout_s=6):
...
def _run_search(page, wid, titles, mode, created, say):
    ...
    PB.open_find_people(page, wid, say)
    _buyer_fill(page, titles, mode, say)
    page.keyboard.press("Escape"); page.wait_for_timeout(700)
    if not created:
        if PB.save_new_and_rename(page, wid, say):
            return True
        return False
    else:
        PB.save_existing(page, say)
        return True
...
def _reopen_view(page, wid):
    ...
    B.focus_table_maybe_empty(page, TABLE); page.wait_for_timeout(900)
    F._close_tools_panel(page)
...
def run_buyer_event(page, wid, name, say, done_segments=None, on_segment=None):
    ...
    created = PB._prepare_people_table(page, wid, say)
