...
gate is right, and so a run can never fire twice.
  ...
  python people_email_event.py <workbook_name>            # both tables
  python people_email_event.py <workbook_name> --skip-run # configure only
"""
...
import apply_people_waterfall_event as A    # noqa: E402
import configure_validate_email_event as V  # noqa: E402
import run_people_table as R                # noqa: E402
...
def do_table(page, entry, table, table_id, say, skip_run=False):
    ...
    if not applied:
        try:
            ...
            if B.table_exists(page, table):
                ...
                if W.find_header_scrolling(page, "WORK EMAIL"):
                    applied = True
    ...
    if not applied:
        r = A.apply_people_template(page, entry, table, False, say,
                                    run_after=False)
    ...
    try:
        r = V.configure(page, entry, say, run_after=False, table=table)
    ...
    rr = R.run_table(page, entry, table, say)
    record_trigger(table_id, entry["workbook_name"], table,
