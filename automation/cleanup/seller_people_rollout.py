...
per "<entity>_<icp>" slug so entities never share state/targets/logs:
  ...
  python seller_people_rollout.py --only "Middle East Coatings Show"   # default exhibitors/labs
  python seller_people_rollout.py --limit 5
  python seller_people_rollout.py --entity sponsors --limit 5          # same flow for Sponsors
"""
...
def main():
    ...
    cfg = seller_people_searches.configure(args.entity, args.icp)   # point the build at this entity/ICP
    slug = cfg.slug()
    ...
    if not target_ids:
        raise SystemExit(
            ...
            f"that have a {cfg.main_table!r} table (see build_workbook_manifest.py).")
