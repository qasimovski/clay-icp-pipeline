...
through correctly as OFF.)
  ...
  python configure_validate_email.py <wid> <name>            # configure only
  python configure_validate_email.py <wid> <name> --run      # + run WORK EMAIL
"""
...
import apply_email_template  # noqa: E402  (_click_opt)
...
def map_email_input(page, say):
    ...
    if not apply_email_template._click_opt_scroll(page, SOURCE_COL, xmin=1290, xmax=1720):
...
def configure(page, entry, say, run_after=False, table=None):
    ...
    if not panel.find_header_scrolling(page, VALIDATE_COL):
        try:
    ...
    panel.open_column_config(page, VALIDATE_COL)
    ...
    v = panel.verify_persisted(page, VALIDATE_COL, SOURCE_COL, say)
    ...
    if run_after:
        ...
        panel.open_column_config(page, SOURCE_COL)
        panel._open_run_settings(page)
        ...
        trig = panel.trigger_run(page, say)
        result["ran"] = trig
