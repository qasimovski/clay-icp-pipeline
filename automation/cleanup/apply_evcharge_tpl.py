...
Unlike apply_v1_event.py, the config-field mapping is DISCOVERED rather than
...
Usage:
  python apply_evcharge_tpl.py --event "EV Auto Show" --inspect  # dump panel fields
  python apply_evcharge_tpl.py --event "EV Auto Show" --dry-run
  python apply_evcharge_tpl.py --event "EV Auto Show" --apply    # apply, then run
  python apply_evcharge_tpl.py --event "EV Auto Show" --run-only # already applied
"""
...
import apply_v1_event as V1  # noqa: E402
...
def apply_template(page, wid, event, inspect, dry_run, say):
    ...
    V1.TEMPLATE = TEMPLATE          # reuse the template-picker flow, our name
    V1._open_v1_retry(page)
...
def main():
    ...
    # run_v1_event resolved TABLE at import time from the default entity config;
    # point it at this entity's table.
    RUN.TABLE = TABLE
    RUN.V1_SIGNATURE = SIGNATURE
    ...
    with common.clay_page(headless=not args.headed) as page:
        ...
        if args.apply or args.run_only:
            res = RUN.run_v1(page, {"workbook_id": wid, "workbook_name": args.event},
                             False, say)
