...
processes, each its own headless browser reusing the saved Clay session.
  ...
  python cleanup_rollout.py --only "Interphex" --dry-run
...
import common          # noqa: E402  (clay_page)
import cleanup_event   # noqa: E402
...
def main():
    ...
    with common.clay_page(headless=not args.headed) as page, \
        ...
        for i, entry in enumerate(pending):
            ...
            try:
                r = cleanup_event.clean_workbook(page, entry, args.dry_run, say)
                results.append(r)
