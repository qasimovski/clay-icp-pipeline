...
progress. Two rules came out of that:
  ...
  * A saved setting is only real if reopening the column shows it. Never trust
  ...
  python fix_workemail_gate.py <wid> <name> --inspect     # read-only report
  python fix_workemail_gate.py <wid> <name>               # repair + verify
"""
...
def repair(page, entry, say, inspect=False):
    ...
    W._set_gate_condition(page, GATE_COLUMN, say)
