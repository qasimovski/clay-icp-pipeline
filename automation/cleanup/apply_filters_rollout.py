...
via filters_state_all.json; the event clears+reapplies so re-runs are safe.
  ...
  python apply_filters_rollout.py --only "ASMS Annual Conference"
  python apply_filters_rollout.py --limit 20        # next 20 not-yet-done
  python apply_filters_rollout.py --dry-run --limit 20
"""
...
import apply_filters_event as A  # noqa: E402
...
def main():
    ...
    state_path = os.path.join(SCRIPT_DIR, f"filters_state_{args.state_suffix}.json")
    state = load_state(state_path)
    pending = [e for e in wbs if state.get(e["workbook_id"], {}).get("status") != "ok"]
    if args.limit:
        pending = pending[: args.limit]

    tag = args.state_suffix if args.ids_file else \
        ("dry" if args.dry_run else ("only" if args.only else "all"))
    ...
    with common.clay_page(headless=not args.headed) as page, \
        ...
        for i, entry in enumerate(pending):
            ...
            try:
                r = A.apply_filters(page, entry, args.dry_run, say)
                results.append(r)
