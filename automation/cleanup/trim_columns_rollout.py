...
right of 'Normalized Country'). Mirrors delete_byproduct_tables_rollout.py: resumable per-shard
state, --dry-run, --only, --limit, --shard/--shards.

  python trim_columns_rollout.py --dry-run
  python trim_columns_rollout.py --only "Interphex"
  python trim_columns_rollout.py                 # single-threaded, all pending
  python trim_columns_rollout.py --shards 2 --shard 0   # (single-threaded proved
...
import trim_columns  # noqa: E402
import pipeline_config as pcfg  # noqa: E402
import state_io           # noqa: E402  (atomic, fail-loud state files)

_SLUG = pcfg.load().slug()  # entity slug (e.g. sponsors_labs) namespaces manifest+state
...
def load_manifest():
    if not os.path.exists(MANIFEST_PATH):
        raise SystemExit(f"no manifest at {MANIFEST_PATH}; run build_workbook_manifest.py")
...
def main():
    ...
    with browser_session.clay_page(headless=not args.headed) as page, \
        ...
        for i, entry in enumerate(pending):
            ...
            try:
                r = trim_columns.clean_workbook_cols(page, entry, args.dry_run, say)
                results.append(r)
